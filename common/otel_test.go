...
func TestContainerIdToServiceName(t *testing.T) {
	...
	assert.Equal(t,
		f("/k8s/telemetry/telemetry-node-agent-np9pk/node-agent"),
		"/k8s/telemetry/telemetry-node-agent")

	assert.Equal(t,
		f("/k8s/telemetry/pyroscope-df884bb79-hhxtv/pyroscope"),
		"/k8s/telemetry/pyroscope")
